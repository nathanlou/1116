...
export default {
  ...
  methods: {
    initChart(arr) {
      ...
      arr.forEach(item => {
        if (dataArr.indexOf(item.companyName) === -1) {
          dataArr.push(item.companyName)
        }
        if (typeArr.indexOf(item.typeName) === -1) {
          typeArr.push(item.typeName)
        }
      })
      typeArr.forEach(type => {
        const seriesObj = { 'name': type, 'type': 'bar', 'label': labelOption }
        const seriesDataArr = []
        dataArr.forEach(company => {
          let flg = true
          arr.forEach(item => {
            if (item.companyName === company && item.typeName === type) {
              flg = false
              seriesDataArr.push(item.num)
              return false
            }
          })
          if (flg) {
            seriesDataArr.push(0)
          }
        })
        seriesObj.data = seriesDataArr
      ...
      this.chart.setOption({
        ...
        legend: {
          data: typeArr// ['云平台控制仪', '砂石分离机', '洗车机', '雾炮车']
        },
        xAxis: [{
          type: 'category',
          axisTick: {
          ...
          data: dataArr// ['客户1', '客户2', '客户3', '客户4', '客户5', '客户6', '客户7', '客户8', '客户9', '客户10']
        }],
        yAxis: [{
          type: 'value'
        }],
