	...
	<div>
		...
		<div class="searchH">
			...
			<div style="display: flex;margin-right: 160px;">
				<el-button class='searchT' type="primary" icon="el-icon-search" round size="mini" @click="companyList">搜索</el-button>
				<el-button class='searchTmini' type="primary" icon="el-icon-search" round size="mini" @click="searchmini">搜索</el-button>
		...
		<el-dialog title="搜索条件" :visible.sync="searchdialogVisible" width="48%" style="text-align: center;">
			<el-form :model="minisearchenterPriseInfo">
				...
				<div style="margin: 0.625rem;">
					...
					<el-select v-model="pcsearchenterPriseInfo.type" placeholder="请选择" style="width: 48%;">
						<el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
					</el-select>
				</div>
				<div style="margin: 0.625rem;">
					<span style="">是否启用：</span>
					<el-select v-model="pcsearchenterPriseInfo.isUser" placeholder="请选择" style="width: 48%;">
						<el-option v-for="item in optionsIS" :key="item.value" :label="item.label" :value="item.value" />
					</el-select>
				</div>
				<div style="margin: 0.625rem;">
					<span class="searchText" style="">按设备数排序：</span>
					<el-select v-model="pcsearchenterPriseInfo.queryOrder" placeholder="请选择" style="width: 42%;">
						<el-option v-for="item in optionsN" :key="item.value" :label="item.label" :value="item.value" />
					</el-select>
				</div>
			</el-form>
		...
		<!--企业信息表单-->
		<el-dialog :title="(form.action ==='save' ?'添加':'修改')+'企业信息'" width="45%" :visible.sync="showForm" style="text-align: center;">
