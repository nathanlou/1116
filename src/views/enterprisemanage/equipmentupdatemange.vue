  ...
  <div>
      ...
      <div style="display: flex;width: 100%;">
        ...
        <div>
          ...
          <el-select v-model="query.queryTypeId" placeholder="请选择" class="searchD_enterName" size="small">
            <el-option v-for="item in optionsEnterType" :key="item.key" :label="item.value" :value="item.key" />
          </el-select>
        </div>
    ...
    <div class="tableD">
      ...
      <el-table ref="multipleTable" :data="tableData" border :default-sort="{prop: 'date', order: 'descending'}" size="small">
          ...
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-form-item label="版本号">
                <span>{{ props.row.versionNo ? props.row.versionNo : '-' }}</span>
              </el-form-item>
        ...
        <el-table-column align="center" class="pcvisible_eltable" label="设备型号" prop="deviceModel" />

        <el-table-column align="version" label="版本号" prop="versionNo" />
        <el-table-column fixed="right" label="操作" align="center">
          <template slot-scope="scope">
            <el-button type="primary" plain round size="mini" @click="controlForm('update',scope.row)">修改升级包</el-button>
            <el-button type="success" plain round size="mini" @click="toPackage(scope.row)">下载升级包</el-button>
            <el-button type="danger" plain round size="mini" @click="deleted(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
